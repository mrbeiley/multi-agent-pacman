...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    self._visited, self.visitedlist = {}, []

  def getStartState(self):
    "Returns the start state (in your state space, not the full Pacman state space)"
    #state is only pacman position and a list of bools if giving info on if corners have been visited
    return self.startState
  ...
  def isGoalState(self, state):
    ...
    #so we can return that as the goal state value
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not hitsWall:
          #test if the successor is a corner and if so,
          #update the corner_bool to reflect that pacman
          #has reached a corner, return that updated corner_bool
          #with the successor
          if (nextx, nexty) in self.corners:
              idx = self.corners.index((nextx, nexty))
              new_bool = list(state[1])
              new_bool[idx] = True
              new_bool = tuple(new_bool)

          #otherwise just return the same corner_bool, since no corner has been visited
          else: new_bool = state[1]

          #add all the successors to the successor list to be returned (ignoring costs)
...
def manhattanDistance(pos1, pos2):
    ...
    #Note: realized after writing this that util.py also has a manhattanDistance, so
    just ended up using this one since it's in the same file. Goes for euclid as well.
    """
    #check that the coordinate system for each point is the same
...
def euclidianDistance(pos1, pos2):
    """
    returns a general euclidiean distance between any 2 points
    in the same n-space
...
def cornersHeuristic(state, problem):
  ...
  man_dist = 0
  #tests if corner has been visited or not, if not gets distance to corner and updates man dist
  for n in range(len(corners)):
      if state[1][n] == False:
 	        corner_dist = manhattanDistance(state[0], corners[n])
    		if corner_dist > man_dist:
			           man_dist = corner_dist
...
def foodHeuristic(state, problem):
  ...
  #return 0 when all food is eaten (goal state)
  else: return 0
...
class ClosestDotSearchAgent(SearchAgent):
  ...
  def findPathToClosestDot(self, gameState):
    ...
    #will use problem to use another search to quickly return the best move to the
    #closest dot
    problem = AnyFoodSearchProblem(gameState)

    #checked all 3 uninformed seaches, all solved quickly, UCS and bfs returned paths of 350
    #which from observation is suboptimal as expected since it only goes for the closest dot
    #and doesnt have a picture of the entire system (see food search)

    #returns a search from a pacman position to the closest dot to that position
